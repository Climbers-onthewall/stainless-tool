...
def plot_product_map(df, x_col, y_col, color_col, size_col, highlight_df=None):
    ...
    sizes = 100 + 400 * (df[size_col] - size_min) / (size_max - size_min)

    scatter = ax.scatter(df[x_col], df[y_col], c=df[color_col], s=sizes,
                         alpha=0.6, cmap='viridis', edgecolors='gray')

    if highlight_df is not None and not highlight_df.empty:
        highlight_sizes = 100 + 400 * (highlight_df[size_col] - size_min) / (size_max - size_min)
        ax.scatter(highlight_df[x_col], highlight_df[y_col], c='red', s=highlight_sizes,
                   alpha=1.0, edgecolors='black', linewidths=2, label='符合筛选条件')
        ax.legend()
    ...
    if x_col == "点蚀当量PREN":
        ax.axvline(x=30, color='red', linestyle='--', label='PREN=30')
    if y_col == "Md30温度(℃)":
        ax.axhline(y=0, color='black', linestyle='--', label='Md30=0℃')

    ax.set_xlabel(x_col, fontsize=14)
    ax.set_ylabel(y_col, fontsize=14)
    ax.set_title(f"不锈钢成分产品地图\n(颜色：{color_col}，大小：{size_col})", fontsize=16)
...
def main():
    ...
    with st.sidebar:
        ...
        composition_ranges = {
            "Cr": st.slider("Cr (wt%)", 15.0, 25.0, (17.0, 19.0), 0.1),
            "Mn": st.slider("Mn (wt%)", 3.0, 15.0, (5.0, 8.0), 0.1),
            "Ni": st.slider("Ni (wt%)", 0.0, 10.0, (2.0, 4.0), 0.1),
            "N": st.slider("N (wt%)", 0.0, 0.6, (0.1, 0.4), 0.01)
        }

        st.subheader("2. 计算步长")
        step_sizes = {
            "Cr": st.number_input("Cr步长", 0.1, 2.0, 0.2),
            "Mn": st.number_input("Mn步长", 0.1, 2.0, 0.5),
            "Ni": st.number_input("Ni步长", 0.1, 2.0, 0.2),
            "N": st.number_input("N步长", 0.01, 0.2, 0.05)
        }
        ...
        fixed_params = {
            "C": st.number_input("C (wt%)", 0.0, 0.2, 0.07),
            "Si": st.number_input("Si (wt%)", 0.0, 2.0, 0.4),
            "P": st.number_input("P (wt%)", 0.0, 0.1, 0.035),
            "S": st.number_input("S (wt%)", 0.0, 0.1, 0.002),
            "Mo": st.number_input("Mo (wt%)", 0.0, 5.0, 0.015),
            "Nb": st.number_input("Nb (wt%)", 0.0, 1.0, 0.001),
            "Ti": st.number_input("Ti (wt%)", 0.0, 1.0, 0.005),
            "Cu": st.number_input("Cu (wt%)", 0.0, 5.0, 1.25),
            "V": st.number_input("V (wt%)", 0.0, 1.0, 0.135),
            "Co": st.number_input("Co (wt%)", 0.0, 5.0, 0.0)
        }
